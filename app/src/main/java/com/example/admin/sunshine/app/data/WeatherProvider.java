...
public class WeatherProvider extends ContentProvider {
    ...
    public int delete(Uri uri, String selection, String[] selectionArgs) {
        ...
        switch(match) {
            case WEATHER: {
                int del = db.delete(WeatherContract.WeatherEntry.TABLE_NAME, selection, selectionArgs);
                if (del > 0)
                    rowsDeleted = del;
                break;
            }
            case LOCATION: {
                int del = db.delete(WeatherContract.LocationEntry.TABLE_NAME, selection, selectionArgs);
                if (del > 0)
                    rowsDeleted = del;
                break;
            }
    ...
    public int update(
        ...
        switch(match) {
            case WEATHER: {
                int updated = db.update(WeatherContract.WeatherEntry.TABLE_NAME, values, selection, selectionArgs);
                if (updated > 0)
                    rowsUpdated = updated;
                break;
            }
            case LOCATION: {
                int updated = db.update(WeatherContract.LocationEntry.TABLE_NAME, values, selection, selectionArgs);
                if (updated > 0)
                    rowsUpdated = updated;
                break;
            }
